...
#ifndef CFG_CPP_MEM_ALIGNMENT
	#define CFG_CPP_MEM_ALIGNMENT 4
#endif
...
gc_object *gc_object_alloc( int size ){
	...
	if( size<64 && (p=gc_cache[size>>3]) ){
		...
		p=(gc_object*)ALIGNED_MALLOC( size, CFG_CPP_MEM_ALIGNMENT );
	}
...
public:
	...
	String( NSString *nsstr ):rep( Rep::alloc([nsstr length]) ){
		unichar *buf=(unichar*)ALIGNED_MALLOC( rep->length * sizeof(unichar), CFG_CPP_MEM_ALIGNMENT );
	...
	public:
		template<class T> CString( const T *data,int length ){
			_rep=(Rep*)ALIGNED_MALLOC( length*sizeof(C)+sizeof(Rep), CFG_CPP_MEM_ALIGNMENT );
			_rep->refs=1;
			_rep->data[length]=0;
		...
		~CString(){
			if( !--_rep->refs ) ALIGNED_FREE( _rep );
		}
...
//			if( atomic_sub( &refs,1 )>1 || this==&nullRep ) return;
		...
		static Rep *alloc( int length ){
			if( !length ) return &nullRep;
			void *p=ALIGNED_MALLOC( sizeof(Rep)+length*sizeof(Char), CFG_CPP_MEM_ALIGNMENT );
			return new(p) Rep( length );
		}
	};
	Rep *rep;
...
int dbg_print( String t ){
	...
	if( n+100>len ){
		len=n+100;
		ALIGNED_FREE( buf );
		buf=(char*)ALIGNED_MALLOC( len, CFG_CPP_MEM_ALIGNMENT );
	}
	buf[n]='\n';
